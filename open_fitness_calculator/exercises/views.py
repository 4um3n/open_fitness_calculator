...
class ListUserExercisesView(FormView):
    ...
    def get_exercises(self):
        ...
        user_exercises = Exercise.objects.filter(profile=self.request.user.profile)
        # admin_exercises = Exercise.objects.filter(is_admin=True) if profile.is_staff or profile.is_admin else []
...
@method_decorator(login_required, name="dispatch")
class UserExerciseView(DetailView, UpdateView):
    model = Exercise
    pk_url_kwarg = "exercise_pk"
    ...
    def post(self, request, *args, **kwargs):
        ...
        return super(UserExerciseView, self).post(request, *args, **kwargs)

    def form_valid(self, form):
        exercises_names = [e.name for e in Exercise.objects.filter(is_admin=True)]
        if form.cleaned_data.get("name") in exercises_names:
            form.add_error("__all__", "Exercise with this name already exist!")
            return self.form_invalid(form)

        return super(UserExerciseView, self).form_valid(form)

    def get_context_data(self, **kwargs):
        self.extra_context = {
            "exercise": self.get_object(),
        }
        return super(UserExerciseView, self).get_context_data(**kwargs)
...
class UpdateUserExerciseView(UpdateView):
    ...
    def get_success_url(self):
        ...
        return reverse_lazy("user exercise", kwargs=kwargs)
